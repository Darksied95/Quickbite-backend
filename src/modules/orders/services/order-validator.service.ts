...
import { IMenuItem, menuItems, menuItems as menuItemsSchema } from "src/database/drizzle.schema";
import { inArray } from "drizzle-orm";

@Injectable()
export class OrderValidator {
    async getValidatedOrderContext(trx: DrizzleTransaction, orderItems: OrderItemDto[]) {

        const menuItemIds = orderItems.map(order => order.menu_item_id)

        const menuItems = await trx.select()
    ...
    private checkStock(orderItems: OrderItemDto[], menuItemsMap: Map<string, IMenuItem>) {
    ...
    private validateItems(menuItemIds: string[], menuItems: IMenuItem[]) {
        ...
        const allMenuItemsHaveSameRestaurantId = menuItems.every(item => item.restaurant_id === restaurantId)

        if (!allMenuItemsHaveSameRestaurantId) {
            throw new BadRequestException("Cannot order from multiple restaurants at once")
        }
