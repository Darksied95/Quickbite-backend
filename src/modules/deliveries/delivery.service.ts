...
export class DeliveryService {
    ...
    return this.db.transaction(async (trx) => {
      ...
      if (order.status !== 'ready_for_pickup') {
        throw new BadRequestException(
          `Cannot accept order that is not ready for pickup, current status is ${order.status}`,
        );
      }
  ...
  async pickupDelivery(order_id: string, user_id: string) {
    return this.db.transaction(async (trx) => {
      ...
      if (!assignment) {
        throw new BadRequestException(
          `No order assignment available for pickup`,
        );
      }
      ...
      const [order] = await trx
        ...
        .where(
          and(eq(orders.id, order_id), eq(orders.status, 'ready_for_pickup')),
        )
        .returning();
  ...
  async deliver(order_id: string, user_id: string) {
    return this.db.transaction(async (trx) => {
      ...
      const [order] = await trx
        ...
        .where(and(eq(orders.id, order_id), eq(orders.status, 'picked_up')))
        .returning();
  ...
  async cancelDelivery(order_id: string, user_id: string, reason: string) {
    return this.db.transaction(async (trx) => {
      ...
      if (!assignment) {
        throw new BadRequestException(
          'No active assignment found or order already picked up',
        );
      }
    });
  }
