...
import {
  type INewMenuCategory,
  IUpdateMenuCategory,
  menuCategories,
} from '../schemas/menu_categories.schema';
...
export class MenuCategoriesRepository {
  ...
  async create(data: INewMenuCategory) {
    const [menuCategory] = await this.db
  ...
  findAll(restaurant_id: string) {
    return this.db.query.menuCategories.findMany({
      where: and(
  ...
  findById(id: string) {
    return this.db.query.menuCategories.findFirst({
