...
export class MenuItemService {
  ...
  async create(restaurant_id: string, items: MenuItem[]) {
    ...
    const existingCategories = await this.menuCategoryService.findByIds(
      category_ids,
      restaurant_id,
    );
  ...
  findAll(filters: { restaurant_id: string; category_id?: string }) {
    const where: { restaurant_id: string; category_id?: string } = {
  ...
  async update(id: string, restaurant_id: string, data: MenuItemUpdate) {
    ...
    if (item.restaurant_id !== restaurant_id) {
      this.logger.warn(
        { menuItemId: id, attemptedBy: restaurant_id },
