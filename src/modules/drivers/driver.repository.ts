...
export class DriverRepository {
    ...
    async findAll() {
        return this.db.query.driverProfiles.findMany({ where: this.nonDeletedDrivers() })
    }

    async findByUserId(user_id: string) {
        return this.db.query.driverProfiles.findFirst({ where: and(eq(driverProfiles.user_id, user_id), this.nonDeletedDrivers()) })
    }

    async update(id: string, data: any) {
        return this.db.update(driverProfiles)
            .set(data)
            .where(and(eq(driverProfiles.id, id), this.nonDeletedDrivers()))
    }
}