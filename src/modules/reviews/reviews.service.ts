  ...
  async create(data: CreateReviewDto, order_id: string, user_id: string) {
    return this.db.transaction(async (trx) => {
      const [order] = await trx
        .select()
        .from(orders)
        .where(and(eq(orders.id, order_id), eq(orders.customer_id, user_id)))
        .for('update');
      ...
      if (order.status !== 'delivered') {
        throw new BadRequestException(
          "Cannot review an order that isn't delivered",
        );
      }

      const [existingReview] = await trx
        .select()
        .from(reviews)
        .where(eq(reviews.order_id, order_id));
